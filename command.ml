...
let rec parse_power (pow : int)=
  ...
  then Printf.printf "Your power cannot be negative, please enter an integer between 0 and 100. > \n" |> 
       fun () -> read_line() |> Parse.parse |> Parse.string_catcher 
                 |> parse_power 
  else 
...
let parse_swing () =
  ...
  let power = read_line () |> Parse.parse |> Parse.string_catcher 
              |> parse_power in 
  ...
  let angle = read_line () |> Parse.parse |> Parse.string_catcher 
              |> parse_angle in 
  ...
  let alignment = read_line () |> Parse.parse |> Parse.string_catcher 
                  |> parse_alignment in
  let swing = {
    club = club;
    power = power;

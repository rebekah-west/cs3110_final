...
let rec parse_power (pow : int)=
  ...
  then Printf.printf "Your power cannot be negative, please enter an integer between 0 and 100. > \n" |> 
       fun () -> read_line() |> Parse.parse |> Parse.string_catcher |> parse_power 
  else 
...
let rec parse_alignment (degrees : int) =
  ...
  then Printf.printf "Your alignment cannot be less than -90, please enter an integer between -90 and 90. > \n" |> 
       fun () -> read_line() |> Parse.parse |> Parse.string_catcher |> parse_alignment 
  else 
...
let parse_swing () =
  ...
  let club = read_line () |> Parse.parse |> parse_club in
  Printf.printf "How hard would you like to hit the ball? Enter an int. > \n";
  let power = read_line () |> Parse.parse |> Parse.string_catcher |> parse_power in 
  ...
  let angle = read_line () |> Parse.parse |> Parse.string_catcher |> parse_angle in 
  ...
  let alignment = read_line () |> Parse.parse |> Parse.string_catcher |> parse_alignment in
  let swing = {
    club = club;
    power = power;
    angle = angle;
